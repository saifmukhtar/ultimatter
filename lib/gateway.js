...
const createMobileGateway = async (options = {}) => {
  ...
  let enableHub = options.enableHub !== undefined ? options.enableHub : 'auto';
  ...
  const mobileUrl = `https://${localIp}:${proxyPort}/?token=${auth.generateExchangeToken()}`;
  const tailscaleUrl = validTailscaleDns ? `https://${validTailscaleDns}:${proxyPort}/?token=${auth.generateExchangeToken()}` : null;
