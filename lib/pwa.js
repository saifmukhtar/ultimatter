...
const getManifest = () => ({
  ...
  description: "Universal Mobile Gateway for AI Coding Agents",
  start_url: "/",
  display: "standalone",
  orientation: "any",
...
const injectPwaMetaTags = (html) => {
  ...
  <script>
    try {
      ...
      var _ultimatterReconnecting = false;
      var _ultimatterWakeup = function() {
        if (_ultimatterReconnecting) return;
        _ultimatterReconnecting = true;
        fetch('/api/bridge-status')
          .then(function(res) { return res.json(); })
          .then(function(data) {
            _ultimatterReconnecting = false;
            window.dispatchEvent(new CustomEvent('ultimatter:network-restored', { detail: data }));
          })
          .catch(function() {
            _ultimatterReconnecting = false;
          });
      };
      window.addEventListener('online', _ultimatterWakeup, true);
