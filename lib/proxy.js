...
/**
 * Initializes and starts the Antimatter reverse proxy and local dashboard servers.
 * 
...
const startProxy = (localIp, tailscaleDns = null) => {
  ...
  proxy.on('error', (err, req, res) => {
    if (res && res.writeHead) {
      ...
      res.end('Antimatter Bridge Error: Upstream IDE Connection Refused');
  ...
  const requestHandler = (req, res) => {
    ...
    if (authResult.redirect) {
      ...
      res.setHeader('Set-Cookie', `mobile_auth=${newSessionCookie}; Path=/; HttpOnly; Secure; SameSite=None`);
  ...
  const secureServer = http2.createSecureServer({
    ...
    SNICallback: (servername, cb) => {
      if (tailscaleDns && servername === tailscaleDns) {
        const tsCerts = getTailscaleCertFiles(tailscaleDns);
    ...
    if (req.url === '/api/ca.pem') {
      ...
      if (rootCaFile && fs.existsSync(rootCaFile)) {
        res.writeHead(200, {
          ...
          'Content-Disposition': 'attachment; filename="antimatter-root-ca.pem"'
        });
  ...
  const handleEaddrinuse = (err) => {
    if (err && err.code === 'EADDRINUSE') {
      ...
      console.log(`\n🚀 Antimatter is already active in background. Re-opening Control Panel...\n`);
      const network = require('./network');
