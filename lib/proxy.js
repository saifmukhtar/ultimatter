...
const startProxy = (localIpOrOptions, optionalTailscaleDns = null) => {
  ...
  let enableHub = 'auto'; // 'auto' | true | false
  ...
  if (typeof localIpOrOptions === 'object' && localIpOrOptions !== null) {
    ...
    enableHub = localIpOrOptions.enableHub !== undefined ? localIpOrOptions.enableHub : 'auto';
  ...
  const requestHandler = (req, res) => {
    ...
    const shouldServeHub = isExplicitHubPath || (
      (pathname === '/') && (
        enableHub === true ||
        (enableHub === 'auto' && availableTargets.length > 1) ||
        (enableHub === 'auto' && availableTargets.length === 0)
      )
    );
