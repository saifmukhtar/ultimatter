...
/** Cache static GPU controller details on startup to minimize hardware bus polling overhead */
let cachedGpuInfo = null;
let cachedGpuPromise = null;
const getGpuInfo = () => {
  if (cachedGpuInfo !== null) return Promise.resolve(cachedGpuInfo);
  if (cachedGpuPromise) return cachedGpuPromise;
  try {
    const si = require('systeminformation');
    cachedGpuPromise = si.graphics()
      .then(gpu => {
        cachedGpuInfo = (gpu && gpu.controllers && gpu.controllers.length > 0) ? gpu.controllers[0] : false;
        return cachedGpuInfo || null;
      })
      .catch(() => {
        cachedGpuInfo = false;
        return null;
...
/**
 * Checks if the client has explicitly chosen an agent target via cookie or query.
 * 
 * @param {object} req - HTTP request object
 * @returns {boolean} True if explicit agent selection exists
 */
const hasExplicitAgentSelected = (req) => {
  const rawCookie = (req.headers && (req.headers['cookie'] || req.headers['Cookie'])) || '';
  if (rawCookie) {
    const cookies = rawCookie.split(';').map(c => c.trim());
    const agentCookie = cookies.find(c => c.startsWith('selected_agent='));
    if (agentCookie) {
      const agentId = decodeURIComponent(agentCookie.split('=')[1] || '').trim();
  ...
  try {
    const url = new URL(req.url, `https://${hostHeader || '127.0.0.1'}`);
    if (url.searchParams.get('agent')) return true;
  } catch (e) {}
  return false;
};
...
const checkAuth = (req) => {
  ...
  if (token) {
    if (auth.verifyExchangeToken(token)) {
 ...
 * Supports both legacy positional signature startProxy(localIp, tailscaleDns) and
 * modern options object startProxy({ localIp, tailscaleDns, proxyPort, dashboardPort, targets, enableControlServer, enableHub }).
 * 
 * @param {string | object} localIpOrOptions - Local IP string or options object
 * @param {string | null} [optionalTailscaleDns=null] - Tailscale MagicDNS domain if available
 * @returns {Promise<import('http2').Http2SecureServer>}
 */
const startProxy = (localIpOrOptions, optionalTailscaleDns = null) => {
  let localIp = '127.0.0.1';
  let tailscaleDns = null;
  let proxyPort = PROXY_PORT;
  let dashboardPort = DASHBOARD_PORT;
  let enableControlServer = true;
  let enableHub = true; // true | false

  if (typeof localIpOrOptions === 'object' && localIpOrOptions !== null) {
    localIp = localIpOrOptions.localIp || '127.0.0.1';
    tailscaleDns = localIpOrOptions.tailscaleDns || null;
    proxyPort = localIpOrOptions.proxyPort || localIpOrOptions.port || PROXY_PORT;
    dashboardPort = localIpOrOptions.dashboardPort || DASHBOARD_PORT;
    enableControlServer = localIpOrOptions.enableControlServer !== false;
    enableHub = localIpOrOptions.enableHub !== undefined ? localIpOrOptions.enableHub : true;
    if (localIpOrOptions.targets) {
      updateTargets(localIpOrOptions.targets);
    } else if (localIpOrOptions.target) {
      updateTargets(localIpOrOptions.target);
    }
  } else {
    localIp = localIpOrOptions || '127.0.0.1';
    tailscaleDns = optionalTailscaleDns;
  }
  ...
  let allowTailscale = network.getAllowTailscale();
  ...
  const requestHandler = (req, res) => {
    ...
    if (pathname === '/api/ca.pem' || pathname === '/api/ca.crt' || pathname === '/api/rootCA.pem' || pathname === '/api/rootCA.crt') {
      ...
      if (rootCaFile && fs.existsSync(rootCaFile)) {
        ...
        const filename = isCrt ? 'ultimatter.crt' : 'ultimatter.pem';
        ...
        const caData = fs.readFileSync(rootCaFile);
        if (caData.includes('BEGIN PRIVATE KEY')) {
          res.writeHead(500, { 'Content-Type': 'text/plain' });
          res.end('Error: Attempted to serve private key');
          return;
        }
        res.end(caData);
        return;
    ...
    if (authResult.hasToken) {
      ...
      try {
        ...
        res.writeHead(302, {
          'Set-Cookie': `mobile_auth=${newSessionCookie}; Path=/; Max-Age=${auth.COOKIE_MAX_AGE_SECONDS}; HttpOnly; Secure; SameSite=Strict`,
          'Location': cleanPath || '/'
        });
        res.end();
        return;
      } catch (e) {
        res.setHeader('Set-Cookie', `mobile_auth=${newSessionCookie}; Path=/; Max-Age=${auth.COOKIE_MAX_AGE_SECONDS}; HttpOnly; Secure; SameSite=Strict`);
      }
    }
    ...
    if (pathname === '/api/auth-token' && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          if (token && auth.verifyExchangeToken(token)) {
            const newSessionCookie = auth.generateSessionCookie();
            res.writeHead(200, {
    ...
    // Agent Dedicated Route: /agent/:id -> persist selection and redirect to /?agent=:id
    if (pathname.startsWith('/agent/')) {
      const parts = pathname.split('/');
      const agentId = parts[2] || '';
      res.writeHead(302, {
        'Set-Cookie': `selected_agent=${encodeURIComponent(agentId)}; Path=/; Max-Age=2592000; Secure; SameSite=Lax`,
        'Location': `/?agent=${encodeURIComponent(agentId)}`
      });
      res.end();
      return;
    }
    ...
    if (pathname === '/api/switch-agent') {
      try {
        ...
        res.setHeader('Set-Cookie', `selected_agent=${encodeURIComponent(targetId)}; Path=/; Max-Age=2592000; Secure; SameSite=Lax`);
        res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    // Determine whether to serve the Mobile Hub or proxy directly to target
    const availableTargets = activeTargets.filter(t => t.enabled !== false);
    let urlObj = null;
    try {
      urlObj = new URL(req.url, `https://${hostHeader || '127.0.0.1'}`);
    } catch (e) {}

    const hasAgentQuery = urlObj ? !!urlObj.searchParams.get('agent') : false;
    const isExplicitHubPath = pathname === '/hub' || pathname === '/agents';
    const isCleanRoot = (pathname === '/') && !hasAgentQuery;
    const shouldServeHub = (enableHub !== false) && (isExplicitHubPath || isCleanRoot);

    if (shouldServeHub) {
      const network = require('./network');
      const disabledList = network.getDisabledAgents();
      const catalogTargets = availableTargets.some(a => !network.AGENT_TARGETS.some(t => t.id === a.id))
        ? availableTargets
        : network.AGENT_TARGETS.filter(t => !disabledList.includes(t.id));
      const enabledActiveTargets = activeTargets.filter(t => !disabledList.includes(t.id));
      res.writeHead(200, {
        'Content-Type': 'text/html; charset=utf-8',
        'Set-Cookie': 'selected_agent=; Path=/; Max-Age=0; Secure; SameSite=Lax'
      });
      res.end(hub.getHubHtml(catalogTargets, enabledActiveTargets));
      return;
    }
      ...
      Promise.all([
        ...
        res.end(JSON.stringify({
          ...
          gpu: gpuInfo ? {
            ...
            vramPercent: gpuInfo.vramDynamic && gpuInfo.memoryTotal
              ? ((gpuInfo.memoryUsed / gpuInfo.memoryTotal) * 100).toFixed(1)
              : 0
          } : null
        }));
      }).catch(err => {
    ...
    if (hasAgentQuery && target) {
      res.setHeader('Set-Cookie', `selected_agent=${encodeURIComponent(target.id)}; Path=/; Max-Age=2592000; SameSite=Lax`);
    }
  ...
  secureServer.closeGateway = () => {
    return new Promise((resolve) => {
      ...
      if (dashboardServer && dashboardServer.close) {
        try { dashboardServer.close(); } catch (e) {}
      }
  ...
  const shutdownGateway = () => {
    try { if (secureServer) secureServer.closeGateway(); } catch (e) {}
    process.exit(0);
  };
  ...
  if (enableControlServer) {
    dashboardServer = startControlServer({
      ...
      setAllowTailscale: (val) => {
        allowTailscale = network.setAllowTailscale(val);
      },
      reloadTlsContext,
      shutdownGateway
    });
  ...
  return new Promise((resolve, reject) => {
    secureServer.on('error', (err) => {
      handleEaddrinuse(err);
      reject(err);
    });

    secureServer.listen(proxyPort, '0.0.0.0', () => {
      const exchangeToken = auth.generateExchangeToken();
      const localUrl = `https://${localIp}:${proxyPort}/?token=${exchangeToken}`;
      const tailscaleUrl = tailscaleDns ? `https://${tailscaleDns}:${proxyPort}/?token=${exchangeToken}` : null;
      ...
      if (enableControlServer) {
        console.log(`✅ Web Control Panel: http://localhost:${dashboardPort}/dashboard`);
      }
      ...
      // Non-blocking background check for latest release in production CLI mode
      if (enableControlServer && process.env.NODE_ENV !== 'test') {
