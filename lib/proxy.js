...
const startProxy = (localIp, tailscaleDns = null) => {
  ...
  // Fix A: Register WebSocket upgrade handler
  secureServer.on('upgrade', upgradeHandler);

  /**
   * Hot-reloads the TLS SecureContext in memory when local domain certs are updated.
   */
    ...
    if (req.method === 'POST' && req.url === '/api/dashboard/set-local-domain') {
      ...
      req.on('end', () => {
        try {
          ...
          // Fix C: Hot-reload TLS certificate into active server memory
          reloadTlsContext();
          res.writeHead(200, { 'Content-Type': 'application/json' });
