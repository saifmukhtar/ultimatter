...
/** Tracks the currently discovered active AI agent target */
let activeTarget = null; // { port: number, name: string, protocol: string }

/**
 * Updates the active upstream agent target and logs status changes.
 * 
 * @param {object | number | null} newTarget - Target object ({ port, name, protocol }) or port number
 */
const updateTarget = (newTarget) => {
  const previousTarget = activeTarget;
  if (typeof newTarget === 'number') {
    activeTarget = newTarget ? { port: newTarget, name: 'Antigravity', protocol: 'https' } : null;
  } else {
    activeTarget = newTarget || null;
  }
  
  const prevPort = previousTarget ? previousTarget.port : null;
  const newPort = activeTarget ? activeTarget.port : null;

  if (activeTarget && newPort !== prevPort) {
    console.log(`\n✅ Connected to ${activeTarget.name} on port: ${activeTarget.port}`);
  } else if (!activeTarget && previousTarget) {
    console.log(`\n⏳ ${previousTarget.name} went offline. Waiting for agent to start...`);
  }
};

const updateTargetPort = updateTarget;
...
<head>
  ...
  <title>Antigravity & OpenCode Bridge</title>
  <style>
...
<body>
  <div class="container">
    ...
    <h1>AI Agent Offline</h1>
    <p>Bridge is active and secured. Launch Antigravity or OpenCode on your PC to connect.</p>
...
const startProxy = (localIp, tailscaleDns = null) => {
  ...
  const requestHandler = (req, res) => {
    ...
    if (req.url === '/api/bridge-status') {
      ...
      res.end(JSON.stringify({ 
        online: !!activeTarget, 
        port: activeTarget ? activeTarget.port : null,
        agentName: activeTarget ? activeTarget.name : null
      }));
      return;
    }
    ...
    cleanHeaders['host'] = `127.0.0.1:${activeTarget.port}`;
    cleanHeaders['origin'] = `${activeTarget.protocol}://127.0.0.1:${activeTarget.port}`;
    cleanHeaders['referer'] = `${activeTarget.protocol}://127.0.0.1:${activeTarget.port}/`;
    ...
    proxy.web(req, res, { target: `${activeTarget.protocol}://127.0.0.1:${activeTarget.port}` });
  };
  ...
  const upgradeHandler = (req, socket, head) => {
    ...
    proxy.ws(req, socket, head, { target: `${activeTarget.protocol}://127.0.0.1:${activeTarget.port}` });
  };
  ...
  const dashboardHandler = (req, res) => {
    ...
    if (req.url === '/api/dashboard/status') {
      ...
      res.end(JSON.stringify({
        ...
        ideOnline: !!activeTarget,
        idePort: activeTarget ? activeTarget.port : null,
        agentName: activeTarget ? activeTarget.name : null,
  ...
  secureServer.listen(PROXY_PORT, '0.0.0.0', () => {
    ...
    console.log(`\n======================================================`);
    console.log(`🚀 ANTIGRAVITY & OPENCODE MOBILE BRIDGE IS LIVE! 🚀`);
    console.log(`======================================================`);
...
module.exports = { startProxy, updateTarget, updateTargetPort, PROXY_PORT, DASHBOARD_PORT };
