 ...
 */
const getHubHtml = (allTargets = [], activeTargets = []) => {
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    .agent-card:hover {
      ...
      box-shadow: 0 14px 34px -10px rgba(0, 0, 0, 0.1);
    }
    .agent-info {
      display: flex;
      align-items: center;
  ...
  <script>
    ...
    document.addEventListener('click', function(e) {
      ...
      if (card && card.dataset.agentId) {
        switchAgent(card.dataset.agentId);
      }
    });
    ...
    const renderActiveAgents = (agents) => {
      ...
      container.innerHTML = activeList.map(function(target) {
        return (
          '<div class="agent-card" data-agent-id="' + escapeHTML(target.id) + '">' +
            '<div class="agent-info">' +
              '<div class="agent-avatar">' + escapeHTML(target.icon || '🤖') + '</div>' +
              '<div class="agent-meta">' +
                '<div class="agent-name-row">' +
                  '<span class="agent-name">' + escapeHTML(target.name) + '</span>' +
                  '<span class="port-tag">:' + target.port + '</span>' +
                '</div>' +
                '<div class="agent-desc">' + escapeHTML(target.description || 'Active AI agent session') + '</div>' +
              '</div>' +
            '</div>' +
        ...
        .then(data => {
          if (data) {
            ...
            if (data.cpu) {
              ...
              if (descEl) descEl.innerHTML = '<span class="vital-pill">⚡ CPU: ' + cpu + '</span> <span class="vital-pill">🧠 RAM: ' + ram + '</span>';
      ...
      <div class="agent-card" style="margin-top: 10px; margin-bottom: 10px; cursor: default; border-left: 3px solid #0284c7;">
        <div class="agent-info">
          ...
          <div class="agent-meta">
            ...
            <div class="agent-desc" id="vitalsDesc">
              <span class="vital-pill" style="opacity:0.6">⚡ CPU: --%</span>
              <span class="vital-pill" style="opacity:0.6">🧠 RAM: --%</span>
            </div>
          </div>
        </div>
    ...
    <div class="ca-download-card">
      <div class="ca-left">
        ...
        <div>
          ...
          <div class="ca-desc">Install on phone for zero-warning HTTPS on Wi-Fi (ultimatter.pem)</div>
        </div>
      </div>
      <div style="display: flex; gap: 6px; align-items: center;">
        <a href="/api/ca.pem" download="ultimatter.pem" class="btn-ca" title="Standard Root CA (.pem)">.PEM ↓</a>
        <a href="/api/ca.crt" download="ultimatter.crt" class="btn-ca" style="background:#f1f5f9;color:#0f172a;border:1px solid #cbd5e1;" title="Android Certificate Installer (.crt)">.CRT ↓</a>
      </div>
    </div>
