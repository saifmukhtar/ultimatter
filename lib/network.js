...
const { CONFIG_DIR, SETTINGS_FILE, LOCAL_CERT_FILE, LOCAL_KEY_FILE } = require('./config');

/**
 * Resolves the configured or default local mDNS domain (e.g. "ultramarine.local").
 * 
 * @returns {string} The local mDNS hostname (e.g. "ultramarine.local")
 */
const getLocalDomain = () => {
  try {
    if (fs.existsSync(SETTINGS_FILE)) {
      const settings = JSON.parse(fs.readFileSync(SETTINGS_FILE, 'utf8'));
      if (settings && settings.localDomain) {
        let domain = settings.localDomain.trim().toLowerCase();
  ...
  const defaultName = os.hostname().toLowerCase().replace(/[^a-z0-9-]/g, '') || 'ultimatter';
  return `${defaultName}.local`;
};

/**
 * Persists a custom local mDNS domain name.
 * 
 * @param {string} customName - The custom name (e.g. "saif-pc" or "saif-pc.local")
...
const setLocalDomain = (customName) => {
  ...
  if (!cleanName.endsWith('.local')) cleanName += '.local';
...
/**
 * Generates local Wi-Fi TLS certificates using mkcert covering both IP and .local domain.
 * 
 * @param {string} localIp - The local machine IP address
 * @param {string} [localDomain] - The local mDNS domain (e.g. "ultramarine.local")
 * @param {boolean} [force=false] - Force regeneration even if files exist
 */
const generateSSLCertificate = (localIp, localDomain = null, force = false) => {
  ...
  console.log(`🔒 Generating trusted SSL certificate for ${localIp} & ${domain} using mkcert...`);
  try {
