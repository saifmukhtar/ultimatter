 ...
 * 
 * @returns {{ state: 'connected' | 'stopped' | 'not_installed', dnsName: string | null, platform: string }}
 */
const getTailscaleState = () => {
  ...
  if (!isInstalled) {
    return { state: 'not_installed', dnsName: null, platform: os.platform() };
  }
  ...
  try {
    ...
    if (status && status.Self && status.Self.DNSName) {
      ...
      return { state: 'connected', dnsName: dns, platform: os.platform() };
    }
  } catch (err) {}

  return { state: 'stopped', dnsName: null, platform: os.platform() };
};
...
const getMkcertBinary = () => {
  ...
  // 1. Check if ~/.config/antimatter/mkcert already exists
...
/**
 * Opens a dedicated floating application window for the Antimatter Control Panel.
 * 
