...
const AGENT_TARGETS = [
  ...
  },
  {
    ...
    defaultPort: 3001,
    probePath: '/health',
    description: 'CloudCLI Autonomous Web Workbench',
    validate: (res) => res.statusCode === 200
  }
];
...
/**
 * Retrieves the persisted remote access (Tailscale) setting. Defaults to true.
 * 
 * @returns {boolean} True if remote access is allowed, false if locked down to LAN only
 */
const getAllowTailscale = () => {
  if (fs.existsSync(SETTINGS_FILE)) {
    try {
      const settings = JSON.parse(fs.readFileSync(SETTINGS_FILE, 'utf8'));
      if (typeof settings.allowTailscale === 'boolean') {
        return settings.allowTailscale;
      }
    } catch (e) {}
  }
  return true;
...
/**
 * Persists the remote access (Tailscale) setting to disk.
 * 
 * @param {boolean} allowed - True to allow remote access, false to restrict to LAN only
 * @returns {boolean} Updated setting value
 */
const setAllowTailscale = (allowed) => {
  let settings = {};
  if (fs.existsSync(SETTINGS_FILE)) {
    try {
      settings = JSON.parse(fs.readFileSync(SETTINGS_FILE, 'utf8'));
  ...
  settings.allowTailscale = allowed !== false;
  fs.writeFileSync(SETTINGS_FILE, JSON.stringify(settings, null, 2), { mode: 0o600 });
  return settings.allowTailscale;
};
...
const generateSSLCertificate = (localIp, localDomain = null, force = false) => {
  ...
  if (!needsRegen && fs.existsSync(LOCAL_CERT_FILE) && fs.existsSync(LOCAL_KEY_FILE)) {
    try {
      const certPem = fs.readFileSync(LOCAL_CERT_FILE);
      const cert = new crypto.X509Certificate(certPem);
      const san = cert.subjectAltName || '';
      const ipMatch = new RegExp('(?:IP Address|DNS Name):\\\\s*' + localIp.replace(/\\./g, '\\\\.') + '(?:,|$)');
      if (!san.match(ipMatch)) {
        console.log(`\\n🔄 Network changed to ${localIp}. Regenerating local certificate...`);
        needsRegen = true;
      }
    } catch (e) {
      needsRegen = true;
  ...
  if (!needsRegen && fs.existsSync(LOCAL_CERT_FILE)) {
    return true;
  }

  const domain = localDomain || getLocalDomain();

  // 1. Try mkcert (primary choice for trusted CA certificate)
  try {
  ...
  // 2. Pure OpenSSL fallback (ensures 100% reliability in tests, containers, and minimal environments)
  try {
    execSync(`openssl req -x509 -newkey rsa:2048 -nodes -sha256 -subj "/CN=${localIp}" -keyout "${LOCAL_KEY_FILE}" -out "${LOCAL_CERT_FILE}" -days 365`, { stdio: 'ignore' });
    if (fs.existsSync(LOCAL_CERT_FILE) && fs.existsSync(LOCAL_KEY_FILE)) {
      return true;
    }
  } catch (e) {}
...
const findAllActiveAgentTargets = () => {
  return new Promise((resolve) => {
    ...
    if (os.platform() === 'darwin') {
      ...
      cmd = `cmd.exe /c "netstat -ano | findstr LISTENING & echo ===TASKLIST=== & tasklist /FO CSV /NH"`;
    } else {
    ...
    exec(cmd, async (err, stdout) => {
      ...
      if (!err && stdout) {
        if (os.platform() === 'win32') {
          const parts = stdout.split('===TASKLIST===');
          if (parts.length === 2) {
            const netstatLines = parts[0].split('\n');
            const tasklistLines = parts[1].split('\n');
            ...
            for (const line of tasklistLines) {
              ...
              if (cols.length >= 2) {
                const pName = cols[0].replace('"', '').trim();
                const pid = cols[1].replace('"', '').trim();
                pidMap[pid] = pName;
              }
            }
            ...
            for (const line of netstatLines) {
              ...
              if (cols.length >= 5) {
                const ipPort = cols[1];
                const pid = cols[4];
                const pName = pidMap[pid] || '';
                for (const target of webTargets) {
                  if (new RegExp(target.processPattern, 'i').test(pName)) {
                    const portMatch = ipPort.match(/(?:127\.0\.0\.1|0\.0\.0\.0|\[::\]|localhost):(\d+)/i);
                    if (portMatch) {
...
const watchIdePort = (callback, fastIntervalMs = 1200, slowIntervalMs = 2000) => {
  ...
  const check = async () => {
    try {
      ...
      const linuxChanged = os.platform() === 'linux' && lastFingerprint !== null && currentFingerprint !== lastFingerprint;
      
      const webTargetsCount = AGENT_TARGETS.filter(t => t.type === 'web').length;
      const hasMissingAgents = lastActiveTargets.length < webTargetsCount;
      const forceHunt = ticks === 1 || linuxChanged || (hasMissingAgents && ticks % 4 === 1);

      let doFullHunt = forceHunt;

      if (!doFullHunt && lastActiveTargets.length > 0) {
        let allAlive = true;
        for (const t of lastActiveTargets) {
          const targetDef = AGENT_TARGETS.find(def => def.id === t.id);
          if (!targetDef || !(await probeTargetPort(t.port, targetDef))) {
            allAlive = false;
            break;
          }
        }
        if (!allAlive) doFullHunt = true;
      }
      ...
      if (doFullHunt) {
        ...
        if (currentTargetKey !== lastTargetKey) {
          ...
          lastActiveTargets = activeTargets;
          steadyCount = 0;
...
const generateTailscaleCert = (dnsName) => {
  ...
  try {
    ...
    execFileSync('tailscale', ['cert', dnsName], { cwd: CONFIG_DIR, stdio: 'pipe' });
