...
const watchIdePort = (callback, fastIntervalMs = 1500, slowIntervalMs = 4500) => {
  ...
  const check = async () => {
    ...
    // Adaptive backoff: 1.5s when hunting, 4.5s normal, 10s when idle and steady
    let nextDelay = fastIntervalMs;
    if (lastTargetKey && lastTargetKey !== '') {
      nextDelay = steadyCount > 5 ? 10000 : slowIntervalMs;
    }
