...
const recordFailedAttempt = (ip) => {
  ...
  // Prevent unbounded memory leak (OOM) by capping tracked IPs to 10,000
  if (!rateLimits.has(cleanIp) && rateLimits.size >= 10000) {
...
const getBannedIpCount = () => {
  ...
  for (const [ip, record] of rateLimits.entries()) {
    if (record.bannedUntil > now) {
      count += 1;
    } else if (record.bannedUntil > 0 || (now - record.lastAttempt > BAN_DURATION_MS)) {
