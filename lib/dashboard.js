...
<body>
  <div class="card">
    ...
    <div class="security-toolbar" style="margin-top: 8px; padding: 8px 12px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; width: 100%; display: flex; align-items: center; justify-content: space-between;">
      <div style="display: flex; flex-direction: column; gap: 2px; text-align: left;">
        <span style="font-size: 12px; font-weight: 600; color: #0f172a;">📱 Floating Switcher Bubble</span>
        <span style="font-size: 10px; color: #64748b;">When OFF (recommended), swipe back to Hub</span>
      </div>
      <div class="security-toggle" onclick="toggleBubble()" style="cursor: pointer;" title="Toggle mobile floating bubble">
        <div id="bubbleSwitchTrack" class="switch-track">
          <div class="switch-thumb"></div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    const toggleBubble = () => {
      fetch('/api/dashboard/toggle-bubble', { method: 'POST' })
        .then(res => res.json())
        .then(data => {
          state.mobileBubbleEnabled = data.mobileBubbleEnabled;
          updateUI();
        })
        .catch(() => {});
    };
            ...
            <div class="smart-body">
              <div class="smart-item">
                ...
                <div class="smart-item-text">
                  <div class="smart-item-title">Google Antigravity</div>
                  <div class="smart-item-desc">Launch the Antigravity IDE on your PC to auto-connect.</div>
                </div>
              </div>
              <div class="smart-item">
                <span class="smart-item-icon">👐</span>
                <div class="smart-item-text">
                  <div class="smart-item-title">OpenCode</div>
                  <div class="smart-item-desc">Start OpenCode web server in 1 line:</div>
                  <div class="code-box" style="margin-top: 6px; margin-bottom: 0;">
                    <span class="code-text" id="opencodeCmdText">opencode web</span>
                    <button id="copyOpencodeBtn" class="copy-small-btn" onclick="copyText('opencode web', 'copyOpencodeBtn')">Copy</button>
                  </div>
                </div>
              </div>
            </div>
    ...
    const pollStatus = () => {
      fetch('/api/dashboard/status')
        ...
        .then(data => {
          ...
          state.mobileBubbleEnabled = Boolean(data.mobileBubbleEnabled);
