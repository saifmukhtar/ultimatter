  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .copy-btn {
      ...
      transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1);
      white-space: nowrap;
      box-shadow: inset 0 1px 1px rgba(255,255,255,0.2), 0 2px 4px rgba(2,132,199,0.2);
    }
    .copy-btn:hover { 
      background: linear-gradient(180deg, #0284c7, #0369a1); 
      box-shadow: inset 0 1px 1px rgba(255,255,255,0.1), 0 4px 10px rgba(2,132,199,0.3);
    }
    ...
    .copy-btn.copied { 
      ...
      box-shadow: inset 0 1px 1px rgba(255,255,255,0.2), 0 2px 4px rgba(5,150,105,0.2);
    }
    .copy-btn.secondary {
      background: #f1f5f9;
      color: #334155;
  ...
  <script>
    ...
    const renderAgentsModalList = () => {
      ...
      list.innerHTML = state.agents.map(function(a) {
        ...
        return '<div class="agent-item-row ' + (isOnline && isEnabled ? 'online' : '') + '" style="' + (!isEnabled ? 'opacity: 0.65;' : '') + '">' +
          ...
          '<div style="display:flex;align-items:center;gap:10px;">' +
            ...
            '</div>' +
            '<div class="switch-track ' + (isEnabled ? 'on' : '') + '" data-id="' + a.id + '" data-enabled="' + !isEnabled + '" onclick="toggleAgent(this.dataset.id, this.dataset.enabled)" style="cursor:pointer;" title="Click to ' + (isEnabled ? 'disable' : 'enable') + ' mobile gateway access for ' + a.name + '">' +
              '<div class="switch-thumb"></div>' +
            '</div>' +
          '</div>' +
        '</div>';
