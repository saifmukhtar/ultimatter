    ...
    attachDrawerItemEvents() {
      ...
      if (hubItem) {
        hubItem.addEventListener('click', () => {
          try { if (navigator.vibrate) navigator.vibrate(8); } catch (e) {}
          const switchBtn = hubItem.querySelector('.badge-switch');
          if (switchBtn) switchBtn.innerText = 'Opening...';
          window.location.assign('/');
        });
      }
      ...
      agentItems.forEach(item => {
        item.addEventListener('click', () => {
          try { if (navigator.vibrate) navigator.vibrate(8); } catch (e) {}
          const switchBtn = item.querySelector('.badge-switch');
          if (switchBtn) {
          ...
          if (agentId) {
            fetch('/api/switch-agent?id=' + encodeURIComponent(agentId), { method: 'POST' })
              .then(() => { window.location.assign('/agent/' + encodeURIComponent(agentId)); })
              .catch(() => { window.location.assign('/agent/' + encodeURIComponent(agentId)); });
          }
        });
      });
    }
